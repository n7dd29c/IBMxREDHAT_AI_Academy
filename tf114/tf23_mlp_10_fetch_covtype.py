...
#2. 모델
w1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([54, 26]), dtype=tf.float32)
b1 = tf.compat.v1.Variable(tf.compat.v1.zeros([26]), dtype=tf.float32)

Layer_1 = tf.nn.leaky_relu(tf.matmul(x, w1) + b1)

w2 = tf.compat.v1.Variable(tf.compat.v1.random_normal([26, 13]), dtype=tf.float32)
b2 = tf.compat.v1.Variable(tf.compat.v1.zeros([13]), dtype=tf.float32)
...
w3 = tf.compat.v1.Variable(tf.compat.v1.random_normal([13, 7]), dtype=tf.float32)
b3 = tf.compat.v1.Variable(tf.compat.v1.zeros([7]), dtype=tf.float32)

hypothesis = tf.nn.softmax(Layer_2 @ w3 + b3)

#3. 컴파일
optm = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-3)

...
#2. 모델
w1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([13, 26]), dtype=tf.float32)
...
w3 = tf.compat.v1.Variable(tf.compat.v1.random_normal([13, 3]), dtype=tf.float32)
b3 = tf.compat.v1.Variable(tf.compat.v1.zeros([3]), dtype=tf.float32)

hypothesis = tf.nn.softmax(Layer_2 @ w3 + b3)

#3. 컴파일
optm = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.02)

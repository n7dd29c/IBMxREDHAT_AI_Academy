...
# 1) 데이터
x_data, y_data = load_breast_cancer(return_X_y=True)  # x:(569,30), y:(569,) in {0,1}
...
y_bin = y_data.astype(np.float32).reshape(-1, 1)

x = tf.compat.v1.placeholder(tf.float32, shape=[None, 30])
y = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])

# 2) 모델 (구조 그대로, 마지막만 시그모이드)
w1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([30, 16], stddev=0.05), dtype=tf.float32)

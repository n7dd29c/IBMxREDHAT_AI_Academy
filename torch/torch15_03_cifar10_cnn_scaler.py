...
transf = tr.Compose([tr.ToTensor(),tr.Resize(64), tr.Normalize((0.5), (0.5))])

# 데이터셋 로드 (transform 적용)
train_dataset = CIFAR10(root=path, train=True, download=True, transform=transf)
test_dataset = CIFAR10(root=path, train=False, download=True, transform=transf)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ...
            nn.LazyLinear(64),
            nn.ReLU(),
            nn.LazyLinear(10) # CIFAR-10은 10개의 클래스
        )

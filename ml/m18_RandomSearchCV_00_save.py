...
#2. 모델
xgb = XGBClassifier()
model = RandomizedSearchCV(xgb, parameters, cv=kfold,   # 42 * 5 = 210
                     verbose=2,
                     n_jobs=-1,
                     refit=True,    # 1번
                     random_state=98,
                     n_iter=11,     # 랜덤으로 뽑아서 쓸 parm의 갯수
                     )  # 총 210번 + 1번 = 211번
...
pd.DataFrame(model.cv_results_).sort_values\
    ('rank_test_score', ascending=True).to_csv(path + 'm18_00_RS_cv_results.csv')

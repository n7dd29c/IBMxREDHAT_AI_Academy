...
for i in thresholds:
    ...
    # 선택된 특성들의 마스크(불리언 배열) 가져오기
    # 선택되지 않은(제거된) 특성들의 마스크를 만듭니다.
    # ~는 True를 False로, False를 True로 반전시킵니다.
    removed_features_mask = ~selection.get_support()

    # 제거된 특성 이름들을 가져옵니다.
    removed_feature_names = feature_names[removed_features_mask]
    ...
    print('삭제 할 컬럼 : ', removed_feature_names)
    print('-------------------------------------------------')
    
    if max_acc<=score:
        max_acc = score
        delete_columns = list(removed_feature_names)

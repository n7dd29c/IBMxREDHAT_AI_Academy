...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,   # 54 * 5 = 270
                     ...
                     random_state=50,
                    #  n_iter=11,
                      factor = 5,    # 데이터는 세 배로 늘리고, 후보군 3분의 1로.
                                    # 훈련 횟수와 상관 없이 모든 데이터는 중복 된다.
                                    # 배수 : min_resource * factor
                                    #       n_candidates / factor
                     min_resources=170,  # 1 iter 때의 최소 훈련 행의 수
...
# - 최적의 파라미터 :  {'learning_rate': 0.1, 'max_depth': 8}
# - best_score :  0.7936961988845671
# - mode.score :  0.8134127346109825
# - accuracy_score :  0.8134127346109825
# - 걸린시간 :  1120.08 초
...
pd.DataFrame(model.cv_results_).sort_values\
    ('rank_test_score', ascending=True).to_csv(path + 'm20_HGS_fetch_covtype_results.csv')
    
import joblib
joblib.dump(model.best_estimator_, path + 'm20_HGS_fetch_covtype_best.joblib')
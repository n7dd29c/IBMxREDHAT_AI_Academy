...
# x = ohe.fit_transform(padding_x)
# print(x.shape)                      # (15, 35)

padding_x = np.array(padding_x)
padding_x = padding_x.reshape(-1, 1)

ohe = OneHotEncoder(sparse_output=False)
padding_x = ohe.fit_transform(padding_x)
print(padding_x.shape)  # (75, 31)

padding_x = padding_x[:, 1:]
print(padding_x.shape)  # (75, 30)

padding_x = padding_x.reshape(15, 5, -1)
print(padding_x.shape)  # (15, 5, 30)

x_train, x_test, y_train, y_test = train_test_split(
    padding_x, label, test_size=0.3, random_state=55
)
print(x_train.shape, x_test.shape)  # (10, 5, 30) (5, 5, 30)

model = Sequential()
model.add(LSTM(32, input_shape=(5,30)))
...
y_pred_pad = pad_sequences(
    y_pred,
    maxlen=5,
    padding='pre'
)
y_pred_ohe = ohe.transform(y_pred_pad)

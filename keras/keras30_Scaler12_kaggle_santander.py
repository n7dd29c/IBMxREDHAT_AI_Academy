...
savepath = './_save/kaggle/santander/'
date = datetime.datetime.now().strftime('%y%m%d_%H%M')
filename = ('{epoch:03d}-{val_loss:.4f}.hdf5')
filepath = ''.join([savepath, 'k30_', date, '_', filename])
mcp = ModelCheckpoint(
    monitor='val_loss',
    mode='auto',
    save_best_only=True,
...
submission_csv['target'] = (y_submit > 0.3).astype(int)
submission_csv.to_csv(path + 'submission_santander_2.csv')

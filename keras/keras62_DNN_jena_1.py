...
jena의 RNN shape를 가져와서 단순 reshape한다

shape = (420000, ts, ft)
reshape = (420000, ts*ft)
...
print(x.shape, y.shape)             # (2919, 144, 13) (2919, 144)
...
#2. 모델구성
model = Sequential()
model.add(Dense(400, input_dim=1872, activation='relu'))
...
model.fit(x_train, y_train, epochs=10000, batch_size=64, validation_split=0.2, verbose=2, callbacks=[es,mcp],)
...
submit_df.to_csv(path + 'jena_안태영_submit_DNN_1.csv', index=False)
